...
function ProjectPopup({
    ...
    return (
        <div onClick={handleOverlayClick}
             className={`project-popup ${isProjectPopupOpened ? 'project-popup_opened' : ''}`}>
            <div className="project-popup__container">
                ...
                <div className='project-popup__slider-wrapper'>
                    <button onClick={handleMoveLeftClick}
                            className='project-popup__scroll-btn project-popup__scroll-btn_left'></button>
                    <ProjectSlider currentProject={currentProject}/>
                    <button onClick={handleMoveRightClick}
                            className='project-popup__scroll-btn project-popup__scroll-btn_right'></button>
                </div>
                <div className='project-popup__bullet-section'>
                    {projects.map((p) => {
                        return (<span id={p.id}
                                      onClick={handleBulletClick}
                                      className={`project-popup__bullet ${p.id === currentProject.id ? 'project-popup__bullet_active' : ''}`}
                                      key={p.id}></span>)
                    })}
                </div>
            </div>
